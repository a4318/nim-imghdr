...
proc checkPNG(value : ptr UncheckedArray[uint8]): ImageType =
    # tests: "\211PNG\r\n" 89 50 4E 47
    return if equalMem(value[0].addr, PNGmagic[0].addr, 4): PNG else: Other
...
proc checkGIF(value : ptr UncheckedArray[uint8]): ImageType =
    # tests: "GIF87a" or "GIF89a"  47,49,46,38,39,61
    return if equalMem(value[0].addr, GIF1magic[0].addr, 2) or equalMem(value[0].addr, GIF2magic[0].addr, 2): GIF else: Other
...
proc checkTIFF(value : ptr UncheckedArray[uint8]): ImageType =
    # tests: "MM" or "II"
    return if equalMem(value[0].addr, TIFF1magic[0].addr, 2) or equalMem(value[0].addr, TIFF2magic[0].addr, 2): TIFF else: Other
...
proc checkRGB(value : ptr UncheckedArray[uint8]): ImageType =
    # tests: "\001\332"
    return if equalMem(value[0].addr, RGBmagic[0].addr, 2): RGB else: Other
...
proc checkPAM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "P7" 50,37
    return if equalMem(value[0].addr, PAMmagic[0].addr, 2): PAM else: Other
...
proc checkBMP(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "BM" 42,4d
...
proc checkRast(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "\x59\xA6\x6A\x95"
    return if equalMem(value[0].addr, Rastmagic[0].addr, 4): Rast else: Other
...
proc checkCRW(value: ptr UncheckedArray[uint8]): ImageType =
     # tests: "II" and "HEAPCCDR" 49, 49 and 48,45,41,50,43,43,44,52
    return if equalMem(value[0].addr, CRW1magic[0].addr, 2) and equalMem(value[6].addr, CRW2magic[0].addr, 8): CRW else: Other
...
proc checkCR2(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: ("II" or "MM") and "CR" (49,49 or 4d,4d) and 43,52
    return if (equalMem(value[0].addr, CR21magic[0].addr, 2) or equalMem(value[0].addr, CR22magic[0].addr, 2) ) and equalMem(value[8].addr, CR23magic[0].addr, 2) : CR2 else: Other
...
proc checkSVG(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "<?xml" 3c,3f,78,6d,6c
    # NOTE: this is a bad way of testing for an SVG, as it can easily fail (eg. extra whitespace before the xml definition)
    # TODO: write a better way. Might require changing from testing the first 32 bytes to testing everything, and using an
    # xml parser for ths one.
    return if equalMem(value[0].addr, SVGmagic[0].addr, 5): SVG else: Other
...
proc checkMRW(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "MRM" 4d,52,4d
    return if equalMem(value[0].addr, MRWmagic[0].addr, 3): MRW else: Other
...
proc checkX3F(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "FOVb" 46,4f,56,62
    return if equalMem(value[0].addr, X3Fmagic[0].addr, 4): X3F else: Other
...
proc checkWEBP(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "RIFF" and "WEBP" 52,49,46,46 57,45,42,50
    return if equalMem(value[0].addr, WEBP1magic[0].addr, 4) or equalMem(value[0].addr, WEBP2magic[0].addr, 4): WEBP else: Other
...
proc checkXCF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "gimp xcf" 67,69,6d,70,20,78,63,66
    return if equalMem(value[0].addr, XCFmagic[0].addr, 8): XCF else: Other
...
proc checkGKSM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "GKSM" 47,4b,53,4d
    return if equalMem(value[0].addr, GKSMmagic[0].addr, 4): GKSM else: Other
...
proc checkPM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "VIEW" 56,49,45,57
    return if equalMem(value[0].addr, PMmagic[0].addr, 4): PM else: Other
...
proc checkFITS(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "SIMPLE" 53,49,4d,50,4c,45
    return if equalMem(value[0].addr, FITSmagic[0].addr, 6): FITS else: Other
...
proc checkXPM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "/* XPM */" 2f,2a,20,58,50,4d,20,2a,2f
    return if equalMem(value[0].addr, XPMmagic[0].addr, 9): XPM else: Other
...
proc checkXPM2(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "! XPM2" 21,20,58,50,4d,32
    return if equalMem(value[0].addr, XPM2magic[0].addr, 6): XPM2 else: Other
...
proc checkPS(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "%!" 25,21
    return if equalMem(value[0].addr, PSmagic[0].addr, 2): PS else: Other
...
proc checkXFIG(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "#FIG" 23,46,49,47
    return if equalMem(value[0].addr, XFIGmagic[0].addr, 4): XFIG else: Other
...
proc checkIRIS(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 01 da
    return if equalMem(value[0].addr, IRISmagic[0].addr, 2): IRIS else: Other
...
proc checkSPIFF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "SPIFF" 53,50,49,46,46
    return if equalMem(value[6].addr, SPIFFmagic[0].addr, 5): SPIFF else: Other
...
proc checkGEM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: EB 3C 90 2A
    return if equalMem(value[0].addr, GEMmagic[0].addr, 4): GEM else: Other
...
proc checkAmiga(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: E3 10 00 01 00 00 00 00
    return if equalMem(value[0].addr, Amigamagic[0].addr, 8): Amiga else: Other
...
proc checkTIB(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "´nhd" c2,b4,6e,68,64
    return if equalMem(value[0].addr, TIBmagic[0].addr, 5): TIB else: Other
...
proc checkJB2(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 97 and "JB2" 97,4a,42,32
    return if equalMem(value[0].addr, JB2magic[0].addr, 4): JB2 else: Other
...
proc checkCIN(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 80 2A 5F D7
    return if equalMem(value[0].addr, CINmagic[0].addr, 4): CIN else: Other
...
proc checkPSP(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "BK" and 00 42,4b,00
    return if equalMem(value[1].addr, PSPmagic[0].addr, 3): PSP else: Other
...
proc checkEXR(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "v/1" and 01 76,2f,31, 01
    return if equalMem(value[0].addr, EXRmagic[0].addr, 4): EXR else: Other
...
proc checkCALS(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "srcdocid:" 73,72,63,64,6f,63,69,64,3a
    return if equalMem(value[0].addr, CALSmagic[0].addr, 8): CALS else: Other
...
proc checkDPX(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "XPDS" or "SDPX" 58,50,44,53 53,44,50,58
    return if equalMem(value[0].addr, DPX1magic[0].addr, 4) or equalMem(value[0].addr, DPX2magic[0].addr, 4) : DPX else: Other
...
proc checkSYM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "Smbl" 53,6d,62,6c
    return if equalMem(value[0].addr, SYMmagic[0].addr, 4): SYM else: Other
...
proc checkSDR(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "SMARTDRW" 53,4d,41,52,54,44,52,57
    return if equalMem(value[0].addr, SDRmagic[0].addr, 8): SDR else: Other
...
proc checkIMG(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "SCMI" 53,43,4d,49
    return if equalMem(value[0].addr, IMGmagic[0].addr, 4): IMG else: Other
...
proc checkADEX(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "PICT" and 00 08 50,49,43,54,00,08
    return if equalMem(value[0].addr, ADEXmagic[0].addr, 6): ADEX else: Other
...
proc checkNITF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "NITF0" 4e,49,54,46,30
    return if equalMem(value[0].addr, NITFmagic[0].addr, 5): NITF else: Other
...
proc checkBigTIFF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "MM" and 00 2B
    return if equalMem(value[0].addr, BigTIFFmagic[0].addr, 4): BigTIFF else: Other
...
proc checkGX2(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "GX2" 47,58,32
    return if equalMem(value[0].addr, GX2magic[0].addr, 3): GX2 else: Other
...
proc checkPAT(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "GPAT" 47,50,41,54
    return if equalMem(value[0].addr, PATmagic[0].addr, 4): PAT else: Other
...
proc checkCPT(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "CPTFILE" or "CPT7FILE" 43,50,54,37,46,49,4c,45
    return if equalMem(value[0].addr, CPTmagic[0].addr, 7) or equalMem(value[0].addr, CPT7magic[0].addr, 8) : CPT else: Other
...
proc checkSYW(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "AMYO" 41,4d,59,4f
    return if equalMem(value[0].addr, SYWmagic[0].addr, 4): SYW else: Other
...
proc checkDWG(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "AC10" 41,43,31,30
    return if equalMem(value[0].addr, DWGmagic[0].addr, 4): DWG else: Other
...
proc checkPSD(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "8BPS" 38,42,50,53
    return if equalMem(value[0].addr, PSDmagic[0].addr, 4): PSD else: Other
...
proc checkFBM(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "%bitmap" #25,62,69,74,6d,61,70
    return if equalMem(value[0].addr, FBMmagic[0].addr, 7): FBM else: Other
...
proc checkHDR(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "#?RADIANCE" #23 3f 52 41 44 49 41 4e 43 45 0a
    return if equalMem(value[0].addr, HDRmagic[0].addr, 11): HDR else: Other
...
proc checkMP(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 0C ED
    return if equalMem(value[0].addr, MPmagic[0].addr, 2): MP else: Other
...
proc checkDRW(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 07
    return if equalMem(value[0].addr, DRWmagic[0].addr, 1): DRW else: Other
...
proc checkMicrografx(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 01 FF 02 04 03 02
    return if equalMem(value[0].addr, Micrografxmagic[0].addr, 6): Micrografx else: Other
...
proc checkPIC(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 01 00 00 00 01
    return if equalMem(value[0].addr, PICmagic[0].addr, 5): PIC else: Other
...
proc checkVDI(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 00 01 00 08 00 01 00 01 01
    return if equalMem(value[0].addr, VDImagic[0].addr, 9): VDI else: Other
...
proc checkICO(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 00 00 01 00
    return if equalMem(value[0].addr, ICOmagic[0].addr, 4): ICO else: Other
...
proc checkJP2(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 00 00 00 0C and "jP"
    ...
    return if equalMem(value[0].addr, JP2magic[0].addr, 6): JP2 else: Other
...
proc checkYCC(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 59 65 60 00
    return if equalMem(value[0].addr, YCCmagic[0].addr, 4): YCC else: Other
...
proc checkFPX(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: 71 B2 39 F4
    return if equalMem(value[0].addr, FPXmagic[0].addr, 4): FPX else: Other
...
proc checkDCX(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: B1 68 DE 3A
    return if equalMem(value[0].addr, DCXmagic[0].addr, 4): DCX else: Other
...
proc checkITC(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: F1 00 40 BB
    return if equalMem(value[0].addr, ITCmagic[0].addr, 4): ITC else: Other
...
proc checkNIFF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "IIN1"
    return if equalMem(value[0].addr, NIFFmagic[0].addr, 4): NIFF else: Other
...
proc checkWMP(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "II" and BC
    return if equalMem(value[0].addr, WMPmagic[0].addr, 3): WMP else: Other
...
proc checkBPG(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "BPG" and FB
    return if equalMem(value[0].addr, BPGmagic[0].addr, 4): BPG else: Other
...
proc checkFLIF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "FLIF"
    return if equalMem(value[0].addr, FLIFmagic[0].addr, 4): FLIF else: Other
...
proc checkPDF(value: ptr UncheckedArray[uint8]): ImageType =
    # tests: "%PDF"
    return if equalMem(value[0].addr, PDFmagic[0].addr, 4): PDF else: Other
